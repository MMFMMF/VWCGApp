...
**Make compliance the default.** SOPs that are easy to follow get followed. If following the SOP requires navigating a poorly organized knowledge base, opening three different systems, or consulting a document that is formatted for readability rather than use -- the friction will erode compliance. Reduce the friction to zero where possible: link to the SOP in the systems where the work happens, format it for scanning rather than reading, build the quality checks into the tools rather than the documentation.
