...
function generateWhyNow(
  ...
  switch (category) {
    ...
    case 'compliance':
      return `${task.title} is a prerequisite for the growth initiatives in later phases. ${ear < 0.7 ? `Your Execution-Ambition Ratio of ${ear.toFixed(2)} means the organization is already stretched — compliance gaps compound this risk.` : `With Organizational Readiness at ${metrics.organizationalReadinessScore}/100, your operational maturity needs this foundation before scaling.`}`;
    case 'marketing':
      return `Strategic coherence is "${coherence}" with an EAR of ${ear.toFixed(2)} — ${task.title} ${coherence === 'aligned' || coherence === 'mostly aligned' ? 'leverages your strategic clarity to drive market awareness' : `must be tightly aligned with vision pillars to avoid diluting an already fragmented strategy (Readiness: ${metrics.organizationalReadinessScore}/100)`}.`;
    case 'training':
    ...
    case 'facilities':
      return `${task.title} affects every employee daily. ${metrics.organizationalReadinessScore < 50 ? `With Organizational Readiness at ${metrics.organizationalReadinessScore}/100, minimizing disruption is critical — the team cannot absorb simultaneous operational and environmental change.` : `With Organizational Readiness at ${metrics.organizationalReadinessScore}/100, complete the relocation during the stabilization window when the team isn't simultaneously executing strategic initiatives.`}`;
    case 'product_launch': {
    ...
    case 'new_offering':
      return `Strategic coherence is "${coherence}" with an EAR of ${ear.toFixed(2)} — ${coherence === 'aligned' || coherence === 'mostly aligned' ? `${task.title} extends your strategic pillars into a new revenue stream` : `resolve the strategic alignment gaps (currently ${coherence}, EAR: ${ear.toFixed(2)}) before ${task.title} to prevent dilution`}. ${ear < 0.7 ? `Execution capacity must improve first.` : ''}`;
    case 'equipment':
      return `${task.title} is a safety and efficiency priority. ${fdi > 5 ? `With Founder Dependency at ${fdi.toFixed(1)}/10, these operational decisions still land on your desk — resolving this creates delegation space.` : `With Organizational Readiness at ${metrics.organizationalReadinessScore}/100, completing operational improvements now prevents them from competing with strategic initiatives for attention.`}`;
